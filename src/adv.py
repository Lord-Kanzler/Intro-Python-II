...
# Takes list of item names and returns a list of the associated ids
...
with open("src/item_list.txt", "r") as f:
    for line in f:
        data = eval(line)
        item_list[data["id"]] = Item(data["name"], data["description"])

# Establish items in rooms
foyer_items = [item_list["rustysword"], item_list["littlefigurine"]]
room["foyer"].add_items(foyer_items)


# Make a new player object that is currently in the 'outside' room.
player_name = input("How would you like to call your character?: ")
...
while True:
    ...
    # Input from user
    inp = input(
        "Decide what you want to do? [Options: Directions:'n', 'e', 's', 'w'  Actions: 'take', 'grab', 'get']"
    )
    ...
    if len(inputs) == 1:
        ...
        if inp in ["i", "inventory"]:
            print("Inventory:")
            [print(line) for line in player1.get_items()]
        ...
        if verb in ["take", "grab", "get"]:
            ...
            removed_items_ids, removed_items = current_room.remove_items(targ_items)
            # Add items to player inventory
            player1.add_items(removed_items)

        if verb in ["remove", "drop", "toss"]:
            # Generate list of target item's id's from input
            targ_items = name_to_id(obj)
            # Attempt to remove target items from room
            removed_item_ids, removed_items = player1.remove_items(targ_items)
            # Add items to room
